...
def pickup():
    ...
    # Data di riferimento selezionata (default: la più recente)
    ref_date_str = request.args.get("ref_date", available[0])

    # Troviamo la data precedente
    ...
    # Dati per il grafico a barre (revenue totale per mese)
...
def caricamento():
    if request.method == "POST":
        ...
        try:
            ...
            # Rinomina colonne flessibili
            col_map = {}
            for col in df.columns:
                if "DATA" in col or "DATE" in col:
                    col_map[col] = "DATA"
                elif col in ("RNS", "ROOMS", "CAMERE", "RN", "ROOM NIGHTS"):
                    col_map[col] = "RNS"
                elif "ADR" in col or "TARIFFA" in col or "RATE" in col:
                    col_map[col] = "ADR"
                elif "REVENUE" in col or "RICAVO" in col or "REV" in col:
                    col_map[col] = "REVENUE"
            df.rename(columns=col_map, inplace=True)

            required = {"DATA", "RNS", "REVENUE"}
            if not required.issubset(set(df.columns)):
                flash(f"Colonne mancanti. Trovate: {list(df.columns)}. Necessarie: DATA, RNS, REVENUE", "error")
                return redirect(url_for("caricamento"))
            ...
            with get_db() as conn:
                # Elimina vecchi dati per questa data di riferimento
